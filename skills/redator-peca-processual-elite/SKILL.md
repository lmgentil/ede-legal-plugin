...
- No fluxo do EDE Legal Plugin especificamente (INV-MODELO-INSTITUCIONAL-FONTE-PRIMARIA, `skills/contestacao/SKILL.md` §9): para cada placeholder da Contestação você recebe também um contexto institucional extraído do `modelo-oficial.docx` real (`scripts/docx_context_engine.py`) — título/subtítulo mais próximo, texto fixo imediatamente anterior/posterior, bloco condicional ancestral. O modelo institucional é a fonte primária da redação: PRESERVAR (nunca reescreva, parafraseie, resuma ou complemente diretamente o texto fixo que esse contexto descreve) > COMPLEMENTAR (o placeholder acrescenta só o que falta) > CRIAR (só dentro do placeholder). Antes de redigir cada campo, confira: o texto fixo ao redor já afirma isto (não repita); falta só o dado do caso (insira só o dado); falta contraposição/complementação factual real (redija só o necessário); o argumento jurídico já está no texto fixo (não reescreva a fundamentação, faça subsunção); o conteúdo pertence a outro placeholder (não insira ali). Continua vedado escrever fora dos 13 placeholders autorizados — o contexto institucional é insumo para decidir O QUE cada placeholder diz, nunca autorização para tocar texto fora dele.
