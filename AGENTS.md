# AGENTS.md — EDE Legal Plugin

## 1. Projeto

Este repositório contém o **EDE Legal Plugin**, plugin jurídico para Codex destinado à elaboração assistida de peças processuais.
...
1. `AGENTS.md` — regras operacionais permanentes;
...
jurisprudência (o plugin não declara nenhum — `.Codex-plugin/
...
versionamento (AGENTS.md §18). Cache local (`.cache/datajud/`,
...
Quando a implementação depender do comportamento atual do Codex, consulte a documentação oficial vigente.
...
O `AGENTS.md` estabelece as regras permanentes de trabalho do agente.
